local x = {}
x["Version"] = "|cFFFAFA44Icy-Veins Stat Priorities:|cFF00EA00 02.04.2017|r"

--[[ Deathknight Blood]]
x[250] = "Balanced: Str > Haste > Vers > Crit > Mast \n DPS Boost: Str > Haste > Crit > Vers > Mast \n Survival: Str > Haste > Vers > Mast > Crit "
--[[ Deathknight Frost]]
x[251] = "Str > Haste 20% > Crit 25% > Vers > (Crit >= Haste 30%) > \n Mast > Haste"
--[[ Deathknight Unholy]]
x[252] = "Str > Mast > Haste 20% > Crit > Vers"

--[[ Druid Balance]]
x[102] = "Haste > Mast > Int > Crit > Vers"
--[[ Druid Feral]]
x[103] =  "Agi > Mast > Crit > Vers > Haste"
--[[ Druid Guardian]]
x[104] = "Vers >= Mast > Haste > Agi > Crit"
--[[ Druid Restoration]]
x[105] = "Raid: Int > Haste > Crit > Mast > Vers \n Dungeon: Int > Haste > Mast > Crit > Vers"

--[[ Hunter Beastmaster]]
x[253] = "Agi > Mast > Haste > Crit > Vers"
--[[ Hunter Marksmanship]]
x[254] = "Mast > Agi > Crit > Haste > Vers"
--[[ Hunter Survival]]
x[255] = "Agi > Haste > Crit > Vers > Mast"

--[[ Mage Arcane]]
x[62] = "Int > Haste 8K > Vers 3K > Crit > Mast"
--[[ Mage Fire]]
x[63] = "Crit > Int > Mast = Haste > Vers"
--[[ Mage Fros]]
x[64] = "Crit 33.34% > Int > Haste >  Vers > Mast"

--[[ Monk Brewmaster]]
x[268] = "Haste to 30% > (Crit = Mast = Vers) > Agi"
--[[ Monk Mistweaver]]
x[270] = "Standard: Int > Crit > Vers > Haste > Mast \n Fist: Int > Vers > Haste >= Crit > Mast \n Dungeon: Int > Haste = Mast > Vers > Crit"
--[[ Monk Windwalker]]
x[269] = "S. Target: Agi > Mast > Crit > Vers > Haste \n M. Target: Agi > Mast > Haste > Crit > Vers"

--[[ Paladin Holy]]
x[65] = "Int > Crit > Vers > Mast > Haste"
--[[ Paladin Protection]]
x[66] = "Survival: Haste > Vers > Mast > Crit > Stam \n DPS Boost: Haste > Crit > Mast > Vers > Stam"
--[[ Paladin Retribution]]
x[70] = "Str > Haste to 22% > Crit = Vers > Mast"

--[[ Priest Discipline]]
x[256] = "Int > Haste > Crit > Mast > Vers"
--[[ Priest Holy]]
x[257] = "Int > Mast > Crit > Haste > Vers"
--[[ Priest Shadow]]
x[258] = "Haste 12K > Mast > Crit > Int > Haste > Vers"

--[[ Rogue Assassination]]
x[259] = "Standard: Agi > Mast > Crit = Vers > Haste \n Exsanguinate: Agi > Vers = Crit > Mast > Haste"
--[[ Rogue Outlaw]]
x[260] = "Agi > Vers > Crit > Mast > Haste"
--[[ Rogue Subtlety]]
x[261] = "Agi > Mast > Vers > Crit > Haste"

--[[ Shaman Elemental]]
x[262] = "Icefury: Int > Crit = Mast > Haste > Vers \n Ascend.: Int > Mast 86.5% > Crit > Haste > Vers \n Dugeons: Int > Haste > Crit > Mast > Vers"
--[[ Shaman Enhancement]]
x[263] = "Agi > Mast > Haste > Crit > Vers"
--[[ Shaman Restoration]]
x[264] = "Int > Mast > Crit > Haste > Vers"

--[[ Warlock Affliction]]
x[265] = "Mast > Crit >= Haste > Int > Vers"
--[[ Warlock Demonology]]
x[266] = "Haste > Crit > Mast > Int > Vers"
--[[ Warlock Destruction]]
x[267] = "Int > Haste > Crit > Vers > Mast"

--[[ Warrior Arms]]
x[71] = "Mast > Haste > Vers > Str > Crit"
--[[ Warrior Fury]]
x[72] = "Haste > Mast > Str > Vers > Crit >"
--[[ Warrior Protection]]
x[73] = "Str > Haste 30% > Mast >= Vers > Crit > Stam"

--[[ Demon Hunter Havoc]]
x[577] = "Crit > Mast > Agi > Haste > Vers"
--[[ Demon Hunter Vengeance]]
x[581] = "Survival: Agi > Vers > Mast >= Haste > Crit \n DPS Boost: Agi > Crit >= Vers >= Mast >= Haste \n Dungeon: Agi > Mast > Vers > Haste > Crit"
stats_Table = x